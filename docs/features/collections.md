...
1. **＋ → New collection** — WhatsApp-shaped **photos first**: one full-width **Designs** tile (camera icon). Phone and desktop open ContinuousCamera: shutter, **Gallery**, and **Designs** on that chrome (not a second tile, and not a row on the phone’s Photo Library menu — that menu is the OS picker). **Designs** there opens the library sheet. **Gallery** is the file picker. If the camera cannot start, a column-width menu offers Photo library or Designs. Album grid = what’s in the pack, capped at **9** with blurred **Load more / +n**. Quiet tip: **Tap a design to edit or add photos.** Optional **Same for all designs** summary row (dashed when empty — skippable) → sheet for shared rate (or range), unit, MOQ, notes, design tags; applies to every design here and **new** ones you add. Pack **name · description · tags** → primary **Create & Publish** or secondary **Save in Draft** — rates not required. Tap a tile → **Update this design**: photo strip + **Add photos** (no per-design photo cap) + fields; **Diff** when different from Same for all; **Use same as all designs** clears overrides. First item is cover. New photos become draft designs under **My designs → Designs · Draft**; library picks join as-is (not overwritten by Same for all). Collection tags cascade to new designs that still have no tags. After create, opens **My designs → Collections** with **Draft** or **Published** filter.
2. On edit: one plain **status line** under the title (`Published · who` / schedule; tap → Visibility when published). Sticky **Update** · **Publish** / Visibility · **Share** (broadcast). **Open** (header) or tap album photos opens the buyer album (`/collections/:id`). Back returns to My Catalog **Collections** tab. Rare actions (**Archive**, Hide) live under header **⋯** only — no status chip pile. Same **Same for all** row + member photo/edit sheet as create. Name · description · tags always on the form. Confirming Same for all applies filled fields to own members that are not Diff; foreign curated designs stay read-only for photos.
