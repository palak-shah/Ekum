...
1. **＋ → New collection** — WhatsApp-shaped **photos first**: one full-width **Designs** tile (camera icon). Phone and desktop open ContinuousCamera: shutter, **Gallery**, and **Designs** on that chrome (not a second tile, and not a row on the phone’s Photo Library menu — that menu is the OS picker). **Designs** there opens the library sheet. **Gallery** is the file picker. If the camera cannot start, a column-width menu offers Photo library or Designs. Album grid = what’s in the pack, capped at **9** with blurred **Load more / +n**. Quiet tip adapts when shared is on. Optional **Same for new photos** summary row (dashed when empty — skippable; accent + **On** when set) → sheet for shared rate (or range), unit, MOQ, notes, design tags. **Done** sets defaults for **new photos** (pending + later camera/gallery); library designs keep their own rates/tags. Adding library designs after shared is on leaves them as-is and marks **Diff** when they differ. Designs that don’t match show a **Diff** badge and accent ring (row also shows **· N Diff**). Pack **name · description · tags** → primary **Create & Publish** or secondary **Save in Draft** — rates not required. Tap a tile → **Update this design**: photo strip + **Add photos** (no per-design photo cap) + fields; **Diff** when different from Same for all; **Use same as all designs** clears overrides. First item is cover. New photos become draft designs under **My designs → Designs · Draft**; library picks join with their existing details. Collection tags cascade to new designs that still have no tags. After create, opens **My designs → Collections** with **Draft** or **Published** filter.
2. On edit: one plain **status line** under the title (`Published · who` / schedule; tap → Visibility when published). Sticky **Update** · **Publish** / Visibility · **Share** (broadcast). **Open** (header) or tap album photos opens the buyer album (`/collections/:id`). Back returns to My Catalog **Collections** tab. Rare actions (**Archive**, Hide) live under header **⋯** only — no status chip pile. Same **Same for all** row (Done for new photos only — no bulk PATCH of members) + member photo/edit sheet as create. Name · description · tags always on the form. Change a member’s rate via tap → member sheet; foreign curated designs stay read-only for photos.
