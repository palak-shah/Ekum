...
1. **＋ → New collection** — **Photos** and/or **Designs** (library opens as a sheet; album grid = what’s in the pack, capped at **9** with blurred **Load more / +n**) → **name · description · tags** → primary **Create & Publish** or secondary **Save in Draft**. Tap a tile to edit that design (same fields as Add designs). First item is cover. **Photos** uses the same capture as **Add designs**: phone ContinuousCamera (Gallery on that chrome); desktop file multi-select. New photos become draft designs; library picks join as-is. Collection tags cascade to new designs unless edited. Designs-only is fine. After create, opens **My designs → Collections** with **Draft** or **Published** filter.
