...
| 48h share link | Share sheet: pinned then recent; quiet **Share a link · 48 hours**. WhatsApp preview: **seller · pack**, blurred collage teaser (`/share-links/:token/og-image`), share text `{Seller} shared {Pack} on Ekum`. Guest `/s/:token` is public. **Everyone** (live): look-only cover + name + designs; **Open on Ekum** → join → this pack. Closed: cover + name + **Request access** (OG may still tease thumbs). Already on Ekum: skip to the real pack. Expires 48h. Not a guest shop. |
