...
1. **＋ → New collection** — **Photos** and/or **Designs** (library opens as a sheet; album grid = what’s in the pack) → name (auto) → **Create**. First item is cover. New photos become draft designs; library picks join as-is. Designs-only is fine.
...
2. Browse member designs; **Select** / long-press → traveling shortlist (survives opening another album) → **Save designs** / **Curate** / **Order** (multi-supplier Order splits via batch). Once **Select** / long-press starts, a floating **Select all** / **Clear** stays under the header (this album’s designs only; other-album shortlist members stay).
...
| Ready | Company-only; never on Explore / public shop |
...
- No ready collections: “Mark a draft ready when the pack is reviewable.”
...
2. Create a draft album → Mark ready → Publish with a Buyer group → drafts become catalog-published with the album.
