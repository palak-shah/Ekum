...
| One living order reference | Each order has **one** trade-thread message (`order_card` / `rate`) that **updates in place**. Inquiry/order create + quote stay **rich bubble cards** (header = `Order #… Action`, kind badge + left rail in **Order teal**, white/surface card for **incoming and outgoing** — teal for accents/CTA only, never solid teal fill by direction; inline thumbs, wide bubble). Alignment still follows mine/theirs. Later status pulses use the **same header grammar** as a compact left-accent chip. Tap → order detail (full timeline). Legacy stacks: UI shows only the latest per order. |
