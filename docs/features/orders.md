...
| Path | Who the buyer’s ticket is with |
|------|-------------------------------|
| **Direct** (Profile default + per share/pack) | Design owner(s). Sharer/pack owner sees **Shared**. Confirm lists each owner name when several. |
| **I handle** | Pack owner / sharer. Mill tickets stay **Waiting** until you **Send** (or **Change** qty/rate, then Send). Seller cannot see or list them before Send. Tickets hide the other end; we do **not** block Connection or chat if they find each other. |

Set on **You → Profile** (default) and override on each forward / publish / curate. Stamp travels on the share (`path=` + message metadata). Needs **I trade on Ekum**. Escape Direct → **Take over** while still requested with no seller quote. See [direct vs handle](../superpowers/specs/2026-08-21-direct-vs-handle-settings-design.md).

**Clarity:** Chat cards: **Forwarded by {name}** when sharer ≠ owner. Open-path list for forwarder: **Shared** (not You sell); counterpart = seller. Toll: Meena sees trader only; Kavita sees trader only until/after Send. Lists otherwise **You buy** / **You sell**. Orders tab is a **unified feed** (orders + samples + returns), newest-first. Top chrome matches Chats/Explore: **search + filter icon + trailing ＋** (new order). Filter menu lists nine statuses (Requested through Cancelled) plus type (Order / Sample / Return). When a menu filter is on, **Showing … · Clear** appears under the search row. **Find** (any query, kind, status, or date) shows **all** matching rows — attention chips Needs you / In progress / **Completed** apply only when Find is empty. `/samples` and `/returns` redirect here with `?kind=`; status deep links use `?status=`. Tiles may show a short staff name when known. Order detail **Timeline** shows a quiet staff name under each step your team performed (quoted, confirmed, dispatched, delivered, placed). Counterparties still see business names only — never your staff. Parties card may still show last-touch staff · date. Timeline includes a **Quoted** step (offered vs asked qty when lowered) while status stays `requested`, and **return** steps after Delivered when returns exist. Status **Confirmed** means agreed / ready to ship — but buyers never see “confirm” verbs: when they accepted a quote, timeline and Parties say **Quote accepted by …**; when the seller locked supply, **Confirmed by …**. CTAs stay role-owned (buyer: Accept quote / Cancel / Deliver; seller: Quote / Confirm lines / Dispatch). Chat keeps **one living reference** per order: rich card for ask-rates / place-order / quote (designs clubbed, `Inquiry #` / `Order #`); later statuses update that same message into a compact chip (`Order #… · Part dispatched`). **View order →** / tap opens the order; **Accept quote** only while `canAcceptQuote`.
