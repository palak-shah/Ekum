...
| Dispatch sheet | **h-12** design thumb (`order-line-photo`) + name + **pending** remaining (accent when &gt; 0); compact qty; **LR** / transporter / parcels optional in footer. Same thumb pattern on Send quote, Confirm/decline, Settle, Edit, Return, mill Send. |
| Settle | Seller-only while **part shipped** (qty mismatch); `POST /orders/:id/settle`; line `quantity` := shipped; status → `settled` (**Completed**). Sheet shows **Dispatched** \| **Pending** columns (pending accent). Timeline **Settled**. **I-handle:** mill Settle rewrites matching parent (Meena) lines; parent → `settled` when **all released** mill subsets are complete (one supplier Settled → parent Settled; two+ with one open → those lines done, rest pending, stay part shipped). **Trader does not Settle** the Manage parent while mill desks exist — mills close it. Stuck parents heal on open/list. |
