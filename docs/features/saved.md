...
4. Open **Saved** from the **Explore header bookmark**, **＋ → Saved**, or More → Saved (`/saved`). Header shows count for the active tab (**N designs** / **N collections**), matching album pages. Tabs **Designs** | **Collections** (default Designs; `?tab=collections`). Grid/Feed toggle on the active tab. Tap design → photo sheet (tap main photo → **PhotoViewer**); tap collection → collection viewer; **×** to remove that bookmark.
...
5. **Explore / Saved:** long-press albums and/or designs into Selection → act from **Your selection** (**Order** · **Curate** · **Bookmark** · **Share**). Share/Bookmark keep album + design types. Order resolves collections (All / Choose) into design lines first. **Curate** with albums uses the same resolve pattern (**Use whole pack** | **Pick designs**); designs-only skips resolve. Primary after expand: **Save draft**. Locked lines stay gray with reason; Curate continues with allowed designs (zero allowed → stay put). **Ask supplier** for pack permission (chat Allow/Deny → per-company product grant) is **Slice B** — see [curate-album-as-is design](../superpowers/specs/2026-09-04-curate-album-as-is-design.md).
