...
2. Open **Saved** from the **Explore header bookmark**, **＋ → Saved**, or More → Saved (`/saved`) — default **grid** (Feed toggle): business, SKU / design count, album collage with **`+N`** on the 4th cell when there are more than 4 designs (same as Explore); tap design → photo sheet (tap main photo → **PhotoViewer**); tap collection → collection viewer; **×** to unsave.
